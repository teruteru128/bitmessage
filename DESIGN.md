...
43. **送信を接続ごとのキュー+EPOLLOUT駆動に作り替える(実装済み・デプロイ済み)**: 2026-09-24、ユーザー依頼。
    ...
    **実装メモ(2026-09-24)**: 4段階で実装した(コミット40b4e58・f44e31c・a600284と、getdata
    ...
    本番での実績(2026-09-26): 初期同期中と見られる新規inbound peerが、約20分にわたって
    getdataを送ってきた。前半は500件、続いて250件ずつを約1分おきに送り、合計4,134件。
    後半はさらに数百件を小刻みに要求した。保留(pending)は最大950件程度で頭打ちになった。
    補充は相手の読む速さに合わせて毎分約210件ずつ進み、前半分は「getdata upload finished:
    4134 sent, 0 not found」で完了した。後半の要求もすべて積み終え、合計約4,950件になった。
    この間、送信停滞・キュー上限による切断と、保留上限による取りこぼしはどちらも起きなかった。
    接続も維持された。旧実装が毎回途中で切り、相手に最初からやり直させていたパターンに、
    設計どおり対応できた。
...
45. **v3/v4の兄弟アドレスが共存するときのgetpubkey応答の取り違えを修正し、v3をv4へ寄せる移行APIを追加(実装済み・デプロイ済み)**:
    ...
    **続き(同日、ユーザーと合意して実装)**: v3の兄弟をv4へ寄せる移行と、v4だけでもv3の要求に
    ...
    追記(2026-09-25、テストの修正): CIのTSanジョブで一括版の「passphrase違いの行が失敗として
    報告される」確認だけが落ちた(failedが0件、converted・remainingは期待どおり)。一括版は
    `ORDER BY created_time, address`で行を読み、created_timeは秒単位で記録される。
    passphrase違いの行とbulkの行が同じ秒に入ると、アドレス順でbulkの行が先になる。すると
    limit=2件の成功で打ち切られ、失敗行に届かない。ローカルでは両者がたいてい別の秒に
    入るので通っていた。本体の処理は失敗行がどこにあっても正しく動くので、テストだけを直した。
    テストが確かめたいのは「失敗行が一覧の先頭に残っても詰まらないこと」なので、その行の
    created_timeをテスト側で明示的に最古へ固定した。全v3行を同じ秒にそろえると修正前の
    テストで同じ確認が落ちることを確かめ、原因がこれであることを確認した。

46. **時計ずれによる切断WARNの間引きと、ハンドラが求めた切断の理由表示(実装済み・デプロイ済み)**:
