...
このリポジトリで作業するAIエージェント向けの最小限の指示。設計・アーキテクチャと
未解決事項(backlog)の全体像は [DESIGN.md](DESIGN.md)(特に§11「次にやること」)、
日付入りの開発経緯(バグの発覚経緯・議論の顛末・PyBitmessage本家との比較調査結果等、
2026-08-24に肥大化したDESIGN.md §11から分離)は [DESIGN-LOG.md](DESIGN-LOG.md)
を参照すること。ポスト量子(ML-DSA/ML-KEM)拡張の提案とプロトタイプ(`src/pq/`、
daemon本体からは未参照)は [DESIGN-PQ.md](DESIGN-PQ.md)。
...
**必ず全件(現在54件)が100%通過することを確認してから完了とすること。** 1件でも失敗した状態で
...
- **ベンチマーク・調査の過程で観測した実行環境の情報を、バージョン管理下のファイルや
  コミットメッセージに書かない。** 同時に動いている他プロセスの名前・用途・PID・稼働期間、
  loadavg、開いているアプリケーション等が該当する。
  **ユーザーが運用している他のノードインスタンスの存在・構成(何本動かしているか、
  どのコミットで動いているか、それぞれが独自のonionサービスを持つか等)も同様に書かない。**
  Bitmessageは匿名性のためのソフトなので、「この運用者は複数ノードを持っていて、片方は
  既知のバグを抱えた古いコミットで動いている」と公開リポジトリから読み取れるのは
  リンカビリティの問題になる。実験構成を文書に残すときは「v5を知らないノードを1本用意する」
  のように**要件だけ**を書き、手元に既にあるかどうかは書かない。測定条件として書いてよいのは
  「ベンチマークとは無関係のCPU負荷が8コア相当あった」のような、**数字の解釈に必要な
  最小限の事実だけ**で、ユーザーがそのマシンで何をしていたかは書かない(実onionアドレスと
  同じく、公開しうる場所へユーザーの実環境を残さない)。
  「個人情報なので削除した」という注記も書かない — 何かがあったと告知して履歴を
  見に行くよう案内するだけで、削除の意味がなくなる。禁止されるのは**特定の削除を
  告知すること**であって、この項目自体のような**一般則としての言明**は指し示す先が
  無いので問題ない(コミットメッセージに「〜を削除した」と経緯を書くのが前者、
  「実環境の情報は書かない」と方針だけを書くのが後者)。
