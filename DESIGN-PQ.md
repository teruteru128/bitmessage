...
約200byte → 約7.8KB に増え、PoWの期待所要時間が16コアで msg 6.4秒→26.2秒、
pubkey 22.6秒→**142秒**になる(§8.4)。暗号処理そのもののCPU時間はむしろ**速くなる**
...
pk_sig, sk_sig = HybridSigKeyGen( seed(start_nonce)     )   # 偶数側、固定
pk_kem, sk_kem = XWingKeyGen(     seed(start_nonce + 1) )   # 奇数側、探索で2ずつ進める
```
...
**決定性生成: KEM鍵ペアをまるごと引き直す。** X-Wingの鍵生成(seed 1本からML-KEMと
X25519の両方)を仕様通り使えるので、構造がいちばん素直になる。実測 0.172 ms/候補 →
期待44ms(§8.2)。署名鍵側を回すより安く(0.340)、v4の「両方を回す」(1.258)の
7分の1で済む。

**ランダム生成: X25519鍵だけを引き直す。** 乱数から作る以上seedの圧縮表現を保つ必要が
無いので、一番安い成分だけを回せる。実測 1本あたり15.5 ms。
この経路で生成したKEM秘密鍵はX-Wingの32byte seed表現を持たない(ML-KEM鍵とX25519鍵が
独立な乱数由来になる)が、鍵ブロブとしては決定性生成のものと同じ形。
...
| 決定性生成 | **両方**のnonceを2ずつ進める | 署名鍵nonceは固定し、**KEM鍵nonceだけ**を2ずつ進める |

v4のランダム生成が既に「片方だけ引き直す」形だったので、v5はその考え方を決定性側にも
広げた形になる。idは全公開鍵のハッシュなので、どれを引き直しても出力は一様に
ばらつき、暗号学的な差は無い。
...
|---|---|---|---|---|
| v4 決定性 | secp256k1 ×2(両方) | 1.258 | 322 ms | 37文字 |
| **v5 決定性(採用)** | **X-Wing鍵ペアまるごと** | **0.172** | **44 ms** | 53文字 |
| (参考) v5 決定性 | 署名鍵ペアまるごと | 0.340 | 87 ms | 53文字 |
| (参考) 探索の下限 | id計算(SHA3-256)のみ | 0.015 | 3.7 ms | — |
| **v5 ランダム(採用)** | **X25519のみ** | — | **15.5 ms**(20本の平均) | 53文字 |
| v5 決定性 null_bytes=2 | X-Wing鍵ペアまるごと | 0.173 | 11.3 s | 52文字 |
| v5 決定性 null_bytes=0 | (探索なし) | 0.567(1回) | 0.6 ms | 54文字 |

- **決定性生成でX-Wing鍵をまるごと引き直しても44ms**。成分分解して一番安いX25519だけを
  回せば23ms程度になるが、その20msのためにX-Wingの鍵生成構造から外れる価値は無いと
  判断した(§4.2.1)。v4の322msと比べれば7分の1である。
- **ランダム生成はX25519だけを引き直すので1本15.5ms。** seedの圧縮表現を保つ必要が
  無いので、こちらでは最安の方式を取れる。
- 探索の下限(id計算だけ)は0.015 ms/候補。SHA3-256の入力が3200byteあるためで、
  鍵生成コストがこれを下回ることはない。
- null_bytes=2(52文字)は期待11.3秒。v4の「もっと短いアドレス」オプションが
  "Spend several minutes of extra computing time" と説明していたのに対し、v5では10秒程度。
...
**ハッシュレートの測り方に注意が要る。** 単一スレッドで測ると 0.57 Mtrial/s だが、
16コア全部を回すと合計 **2.34 Mtrial/s** にしかならない(単純な16倍より**3.9倍低い**)。
全コア負荷時のクロック低下とSMT(論理16コアが物理8コア相当)のため。PoWは常に
全コアを使い切る処理なので、この条件で測った値でないと推定が4倍ずれる
(実際に一度ずらした。§DESIGN-LOG 2026-09-17)。

以下の「期待所要」は 2.34 Mtrial/s 基準。PoWの所要時間は指数分布に従うので、
...
|---|---|---|---|---|---|
| v4 pubkey (TTL 28日) | 396 | 5.29e7 | **22.6 s** | 35.1 s | 3.15 M/s |
| **v5 pubkey (TTL 28日)** | 7776 | 3.33e8 | **142.2 s** | 81.2 s | 2.50 M/s |
| v4 msg 本文1000 (4日) | 1404 | 1.51e7 | **6.4 s** | 0.9 s | 2.57 M/s |
| **v5 msg 本文1000 (4日)** | 8783 | 6.14e7 | **26.2 s** | 10.5 s | 1.95 M/s |
| v5 getpubkey (28日) | 54 | 4.00e7 | **17.1 s** | 38.3 s | 2.32 M/s |
| v5 broadcast 本文1000 (4日) | 8782 | 6.14e7 | **26.2 s** | 4.2 s | 3.64 M/s |

読み取れること:

1. **v4 pubkeyの期待22.6秒は、DESIGN.md §5.1が実運用で観測していた「実測20秒超」と
   一致する。** 校正が正しいことの裏付けになっている。
2. **v5 pubkeyの期待142秒(約2.4分)がPQ化の最大の実務的コスト。** v4の6.3倍。
   アドレスを作るたびに、16コアを2分半使い切ることになる。
3. msgは26.2秒(v4の4.1倍)。送信のたびにこれを払う。TTLを短くすれば比例して下がる
   (TTLの項が支配的なので、4日→1日で約1/4)。
4. **getpubkeyは54byteしかないのに17.1秒かかる。** `payloadLengthExtraBytes=1000` が
